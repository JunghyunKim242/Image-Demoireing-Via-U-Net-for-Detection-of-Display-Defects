...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        # print('x.seblock',x.shape)
        # print('x.size(2)',x.size(2))        
        w = F.avg_pool2d(x, (x.size(2),x.size(3)))
        # print('w',w.shape)
        w = F.relu(self.fc1(w))
...
# ==============================================================================
    ...
    def forward(self, x):
        ...
        for ii in range(self.final_cfg["NUM_BRANCHES"]):
            ...
            #####
            # p1d = (1,1,1,1)
            # edge_X_padding = F.pad(edge_X, p1d, "constant", 0)
            # edges.append(edge_X_padding)
            #####
            edges.append(edge_X)
...
def get_pose_net(pretrained = None, **kwargs):
    cfg.merge_from_file("Net/config/cfg.yaml")
