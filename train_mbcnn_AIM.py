...
from data_loader.dataset import AIM19_train_MBCNN, AIM19_train_MBCNN
...
#################
def train_MBCNN_AIM(args, model):
    ...
    Moiredata_train = AIM19_train_MBCNN(args.traindata_path)
    ...
    Moiredata_test = AIM19_train_MBCNN(args.testdata_path)
...
def test(model, dataloader, epoch, args):
    ...
    for ii, (val_moires, val_clears, labels) in tqdm(enumerate(dataloader)):
        ...
        for jj in range(batch):
            ...
            psnr_output_meter255.add(psnr_output_individual)
